#!/bin/bash
# Usage: ./create_icns.sh source_image.png output_name
# Example: ./create_icns.sh icon.png FlowParquet

SRC_IMG=$1
OUT_NAME=$2

if [ -z "$SRC_IMG" ] || [ -z "$OUT_NAME" ]; then
    echo "Usage: ./create_icns.sh source_image.png output_name"
    exit 1
fi

ICONSET_DIR="${OUT_NAME}.iconset"
mkdir -p "$ICONSET_DIR"
